...
#ifndef _EmuTFMonitor_h_
#define _EmuTFMonitor_h_
...
class EmuTFMonitorTimerTask: public Task
{

public:
  EmuTFMonitorTimerTask(): Task("EmuTFMonitorTimerTask")
  {
    timerDelay=60;
    plotter = NULL;
    fname = "";
  ...
  EmuTFMonitorTimerTask(std::string name): Task("EmuTFMonitorTimerTask"), fname(name)
  {
    timerDelay=120;
    plotter = NULL;
    fActive=false;
  }
  ~EmuTFMonitorTimerTask() { }

  void setTimer(int delay)
  {
    timerDelay = delay;
  }
  void setPlotter(EmuTFPlotter* pl)
  {
    plotter = pl;
  }
...
public:
  ...
  EmuTFMonitor(xdaq::ApplicationStub* c) throw(xdaq::exception::Exception);
  ~EmuTFMonitor();
