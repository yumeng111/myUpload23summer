...
static unsigned long int proc_previousjiffies_2;
...
#if SLC_VERSION == 7
/* static struct mutex mutex_eth2; */
#endif
...
#ifdef DEBUG_PRINT
  printk(KERN_INFO "-------------------------------------------------------------\n");
  printk(KERN_INFO "ETH2 icnt %d len %d \n",icnt,skb->len);
#endif
  for(i=0;i<icnt;i++){
    *(unsigned long int *)(buf_pnt_2+i*8)=*(unsigned long int *)(skb->data+i*8+SKB_OFFSET);
  }
...
/* #endif */
/*   for(i=0;i<icnt;i++){ */
/*     *(unsigned int *)(buf_pnt_2+i*4)=*(unsigned int *)(skb->data+i*4+SKB_OFFSET); */
/*   } */
...
// calculate rate for proc
   if(flag_rate_2==1){
     proc_rsum_2 += skb->len+SKB_EXTRA;
     if ( proc_rfirst_2==0 ){
       proc_previousjiffies_2 = get_jiffies_64();
       proc_rfirst_2=1;
     }
     else{
       delta_jiffies = get_jiffies_64() - proc_previousjiffies_2;
       if ( delta_jiffies > HZ ){ // Update ~every second
   	 proc_rate_2 = ( HZ * proc_rsum_2<<3 ) / delta_jiffies; // bit/s (proc_rsum_2 counts bytes; HZ=jiffies/s)
   	 proc_previousjiffies_2 = get_jiffies_64();
   	 proc_rsum_2 = 0;
       }
     }
   ...
   */
...
#ifdef DEBUG_PRINT
   // DEBUG START
   printk(KERN_INFO "ETH2 Length %d, packet_count %d, proc_last_pmissing %d, proc_pmissing %ld, missing_mask 0x%04x\n",skb->len,packet_count,proc_last_pmissing_2,proc_pmissing_2,missing_mask);
   ...
   if(flag_eevent_2==1){
     ...
     if ( skb->len+SKB_EXTRA <= 64 ){
       ...
       eevent_mask=0x4000;
#ifdef DEBUG_PRINT
       printk(KERN_INFO "ETH2  %d-byte packet ends event\n",skb->len+SKB_EXTRA);
#endif
     } // if ( skb->len+SKB_EXTRA <= 64 )
     else{
...
#ifdef DEBUG_PRINT
       printk(KERN_INFO "ETH2  DDU trailer?  %04x %04x %04x %04x\n",end1,end2,end3,end4);
#endif
...
#endif
{
	...
	len += sprintf(schar_proc_string_2+len, "  recieve\t\t%ld packets\n",proc_rpackets_2);
      ...
      len += sprintf(schar_proc_string_2+len, "  rate  \t\t\t%6ld KiB/s (%10ld bit/s)\n",proc_rate_2>>13,proc_rate_2);
