...
#else
static long schar_ioctl_4(struct file *file, unsigned int cmd, unsigned long arg);
#endif
...
static char *buf_eend_4;
static char *buf_pnt_4;

static char *ring_start_4;
static unsigned long int ring_size_4;
static unsigned long int ring_pnt_4;
static int short ring_loop_4;

static char *tail_start_4;

static unsigned long int proc_rpackets_4;
static unsigned long int proc_rbytesL_4;
static unsigned short int proc_rbytesH_4;
static unsigned long int proc_pmissing_4;
static unsigned short int proc_last_pmissing_4;
...
static int flag_pmissing_4={1};   // check for missing packets
static int flag_eevent_4={1};     // check for end of events
static int flag_rate_4={1};       // turn on rate monitoring

static spinlock_t eth_lock;

#if SLC_VERSION == 7
/* static struct mutex mutex_eth4; */
#endif
...
{ 
  ...
  printk(KERN_INFO "LSD: buf_start_4 is allocated \n");
  buf_end_4=buf_start_4+(BIGPHYS_PAGES_4-RING_PAGES_4)*PAGE_SIZE-MAXPACKET_4;
  buf_eend_4=buf_start_4+(BIGPHYS_PAGES_4-RING_PAGES_4)*PAGE_SIZE-TAILPOS-MAXEVENT_4;
  ring_start_4=buf_start_4+(BIGPHYS_PAGES_4-RING_PAGES_4)*PAGE_SIZE;
  ring_size_4=(RING_PAGES_4*PAGE_SIZE-RING_ENTRY_LENGTH-TAILMEM)/RING_ENTRY_LENGTH;
  tail_start_4=buf_start_4+BIGPHYS_PAGES_4*PAGE_SIZE-TAILPOS;
  schar_reset_4();
  return 0;
}
...
// calculate rate for proc
   if(flag_rate_4==1){
     ...
     if ( proc_rfirst_4==0 ){
       ...
       delta_jiffies = get_jiffies_64() - proc_previousjiffies_4;
       if ( delta_jiffies > HZ ){ // Update ~every second
   	 proc_rate_4 = ( HZ * proc_rsum_4<<3 ) / delta_jiffies; // bit/s (proc_rsum_4 counts bytes; HZ=jiffies/s)
   	 proc_previousjiffies_4 = get_jiffies_64();
   	 proc_rsum_4 = 0;
       }
     }
   ...
   */
   if(proc_last_pmissing_4+1!=packet_count){
     if(ring_pnt_4!=0||ring_loop_4!=0){
        if(proc_last_pmissing_4==0xffff){
           if(packet_count!=0){proc_pmissing_4=proc_pmissing_4+1;missing_mask=0x8000;}
        }else{
           proc_pmissing_4=proc_pmissing_4+1;missing_mask=0x8000;
        }
     }
   }
...
#ifdef DEBUG_PRINT
   // DEBUG START
   printk(KERN_INFO "ETH4 Length %d, packet_count %d, proc_last_pmissing %d, proc_pmissing %ld, missing_mask 0x%04x\n",skb->len,packet_count,proc_last_pmissing_4,proc_pmissing_4,missing_mask);
   ...
   if(flag_eevent_4==1){
     ...
     if ( skb->len+SKB_EXTRA <= 64 ){
       ...
       eevent_mask=0x4000;
#ifdef DEBUG_PRINT
       printk(KERN_INFO "ETH4  %d-byte packet ends event\n",skb->len+SKB_EXTRA);
#endif
     } // if ( skb->len+SKB_EXTRA <= 64 )
     else{
...
#ifdef DEBUG_PRINT
       printk(KERN_INFO "ETH4  DDU trailer?  %04x %04x %04x %04x\n",end1,end2,end3,end4);
#endif
...
// increment ring and buf pointers
 ...
 // if((buf_pnt_4 > buf_end_4)||(ring_pnt_4>=ring_size_4)){
   if(((eevent_mask==0x4000)&&(buf_pnt_4 > buf_eend_4))||(buf_pnt_4 > buf_end_4)||(ring_pnt_4>=ring_size_4)){
     ring_pnt_4=0;
     ring_loop_4=ring_loop_4+1;
     buf_pnt_4=buf_start_4;
  }

// write data to tail word
  *(unsigned long int *)tail_start_4=buf_pnt_4-buf_start_4;
...
#else
static long schar_ioctl_4(struct file *file,
		       unsigned int cmd, unsigned long arg)
#endif
{
unsigned long int pagsiz;
 printk(KERN_INFO " ioctl: Entered \n");
	...
	case SCHAR_PMISSING:{
		  if(flag_pmissing_4==0)flag_pmissing_4=1;
		  if(flag_pmissing_4>0)flag_pmissing_4=0;
		      return 0;
            }

	case SCHAR_EEVENT:{
		  if(flag_eevent_4==0)flag_eevent_4=1;
		  if(flag_eevent_4>0)flag_eevent_4=0;
		      return 0;
            }
...
#endif
{
	...
	v2.6remove */
	len += sprintf(schar_proc_string_4, "GIGABIT DRIVER ETH4: mm \n\n");
	...
	len += sprintf(schar_proc_string_4+len, "  recieve\t\t%ld packets\n",proc_rpackets_4);
      ...
      len += sprintf(schar_proc_string_4+len, "  rate  \t\t\t%6ld KiB/s (%10ld bit/s)\n",proc_rate_4>>13,proc_rate_4);
	...
	len += sprintf(schar_proc_string_4+len, " TRANSMIT: \n");
      ...
      len += sprintf(schar_proc_string_4+len, "  transmit    \t\t\t%d%08ld bytes\n\n",proc_tbytesH_4,proc_tbytesL_4); 
  	len += sprintf(schar_proc_string_4+len, " ERROR STATISTICS: \n");
      len += sprintf(schar_proc_string_4+len, "  missing packets \t\t%ld\n",proc_pmissing_4);
	*lenp = len;
#if   SLC_VERSION == 5
...
#elif SLC_VERSION == 7
  dev=dev_get_by_name(&init_net, __ETH4__ );
#endif
  err=-ENODEV;
  if (dev == NULL)
   goto out_unlock;
...
proc_rbytesH_4=0;
proc_pmissing_4=0;
proc_last_pmissing_4=0;

proc_tpackets_4=0;
proc_tbytesL_4=0;
proc_tbytesH_4=0;

*(unsigned long int *)tail_start_4=0;
