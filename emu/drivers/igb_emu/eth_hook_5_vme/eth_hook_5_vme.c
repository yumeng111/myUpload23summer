...
#else
static long schar_ioctl_5(struct file *file, unsigned int cmd, unsigned long arg);
#endif
...
// fill bigphys memory and increment counters
  ...
  if(nbufw+9000> MMT_BUF_SIZE){
    printk(KERN_CRIT "eth_hook_5: Catastrophic Error! Overwrote memory (nbufw+9000=%d > MMT_BUF_SIZE=%d)! Reset!\n", nbufw+9000, MMT_BUF_SIZE );
	pack_left=0;
	bufw=pnt_ring;
	nbufw=0;
	bufr=pnt_ring;
...
#else
static long schar_ioctl_5(struct file *file,
		       unsigned int cmd, unsigned long arg)
#endif
{
...
#ifdef DEBUG_PRINT
    printk(KERN_ERR "ETH5 schar_read_5 signal_pending(current)\n" ); 
#endif
    return -EINTR;
  }
...
#ifdef DEBUG_PRINT
	  printk(KERN_ERR "ETH5 schar_read_5 copy_to_user(buf,bufr2,lsend=%d)\n",lsend); 
#endif
	  return -EFAULT;
	}
        count=lsend;
...
#ifdef DEBUG_PRINT
	printk(KERN_INFO "ETH5 schar_read_5 read: %d pack_left: %d\n", count, pack_left ); 
#endif
	return count;
  }else{
          tbuf[0]=4;
...
#ifdef DEBUG_PRINT
	    printk(KERN_ERR "ETH5 schar_read_5 copy_to_user(buf,bufr2,tsend=%d)\n",tsend); 
#endif
	    return -EFAULT; 
	  }
#ifdef DEBUG_PRINT
	printk(KERN_INFO "ETH5 schar_read_5 read: %d when no packets left\n", count ); 
#endif
   	return count; 
  }
  if(endcond==1){
	  ...
	  tsend=6;
          ...
          if (copy_to_user(buf,tbuf,tsend)){
#ifdef DEBUG_PRINT
	  printk(KERN_ERR "ETH5 schar_read_5 copy_to_user(buf,bufr2,tsend=%d) with endcond==1\n",tsend); 
#endif
		return -EFAULT;
	  }
  }
#ifdef DEBUG_PRINT
  printk(KERN_INFO "ETH5 schar_read_5 read: %d endcond: %d\n", count, endcond ); 
#endif
  return count;
}
...
#elif SLC_VERSION == 7
  dev=dev_get_by_name(&init_net, __ETH5__ );
#endif
  err=-ENODEV;
  if (dev == NULL)
   goto out_unlock;
...
#ifdef DEBUG_PRINT
  printk(KERN_INFO "ETH5 schar_write_5 count: %d\n", count);
  printk(KERN_INFO "ETH5 device name: %s hard_header_len: %d\n", dev->name, dev->hard_header_len);
  /* printk(KERN_INFO "eth_hook_5_vme BITS_PER_LONG=%d\n", BITS_PER_LONG); */
#endif
...
*/
  ...
  print_skb( "ETH5 after dev_alloc_skb", skb );
/*
...
*/
   ...
   print_skb( "ETH5 after skb_reserve", skb );
#if   SLC_VERSION == 5
   skb->nh.raw = skb->data;
#else
   skb_reset_network_header(skb);
#endif
   print_skb( "ETH5 after skb_reset_network_header", skb );
...
#endif
			print_skb( "ETH5 after skb_reset_tail_pointer", skb );
...
/* Returns -EFAULT on error */
   ...
   print_skb( "ETH5 after copy_from_user", skb );
