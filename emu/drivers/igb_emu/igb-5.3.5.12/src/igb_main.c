...
#define DRV_SUMMARY	"Intel(R) Gigabit Ethernet Linux Driver with hooks for Emu DDU spy readout and PCrate control"
...
#endif
                  {
                    if      (strcmp(netdev_ring(rx_ring)->name, __ETH2__ )==0){
                      netif_rx_hook_2(skb);
                    }else if(strcmp(netdev_ring(rx_ring)->name, __ETH3__ )==0){
                      netif_rx_hook_3(skb);
                    }else if(strcmp(netdev_ring(rx_ring)->name, __ETH4__ )==0){
                      netif_rx_hook_4(skb);
                    }else if(strcmp(netdev_ring(rx_ring)->name, __ETH5__ )==0){
                      netif_rx_hook_5(skb);
                    }else{
#ifdef HAVE_VLAN_RX_REGISTER
			igb_receive_skb(q_vector, skb);
