...
static long schar_ioctl_2(struct file *file, unsigned int cmd, unsigned long arg);
...
// write length to first word 
  ...
  { printk(KERN_INFO "eth_hook_2: out of memory, incoming packet dropped! \n");
    pack_drop++;
...
// fill bigphys memory and increment counters
  ...
  if(nbufw+9000> MMT_BUF_SIZE){
    printk(KERN_CRIT "eth_hook_2: Catastrophic Error! Overwrote memory (nbufw+9000=%d > MMT_BUF_SIZE=%d)! Reset!\n", nbufw+9000, MMT_BUF_SIZE );
	pack_left=0;
	bufw=pnt_ring;
	nbufw=0;
	bufr=pnt_ring;
...
static long schar_ioctl_2(struct file *file,
...
#endif
{
	...
	v2.6remove */
	len += sprintf(schar_proc_string_2, "GIGABIT SIMPLE CHAR DRIVER ETH2\n\n");
	len += sprintf(schar_proc_string_2+len, " LEFT TO READ: \n");
        len += sprintf(schar_proc_string_2+len," pack_left\t\t%d packets\n",pack_left);
 	len += sprintf(schar_proc_string_2+len, " wakestatus\t\t%d\n",wakestatus);
        len += sprintf(schar_proc_string_2+len, " wakecond\t\t%d\n",wakecond);
	len += sprintf(schar_proc_string_2+len, " endcond\t\t%d\n",endcond);
	len += sprintf(schar_proc_string_2+len, " error\t\t%d\n",ERROR);	
...
{ unsigned short int tbuf[3];
  ...
  wakestatus=1;
  wait_event_interruptible_timeout(schar_wq_2,(wakecond!=0),rd_tmo);
  wakestatus=3;
  if (signal_pending(current)){
#ifdef DEBUG_PRINT
    printk(KERN_ERR "ETH2 schar_read_2 signal_pending(current)\n" ); 
#endif
    return -EINTR;
  }
...
#ifdef DEBUG_PRINT
	  printk(KERN_ERR "ETH2 schar_read_2 copy_to_user(buf,bufr2,lsend=%d)\n",lsend); 
#endif
	  return -EFAULT;
	}
        count=lsend;
...
#ifdef DEBUG_PRINT
	printk(KERN_INFO "ETH2 schar_read_2 read: %d pack_left: %d\n", count, pack_left ); 
#endif
	return count;
  }else{
          tbuf[0]=4;
...
#ifdef DEBUG_PRINT
	    printk(KERN_ERR "ETH2 schar_read_2 copy_to_user(buf,bufr2,tsend=%d)\n",tsend); 
#endif
	    return -EFAULT; 
	  }
#ifdef DEBUG_PRINT
	printk(KERN_INFO "ETH2 schar_read_2 read: %d when no packets left\n", count ); 
#endif
   	return count; 
  }
  if(endcond==1){
	  ...
	  tsend=6;
          ...
          if (copy_to_user(buf,tbuf,tsend)){
#ifdef DEBUG_PRINT
	  printk(KERN_ERR "ETH2 schar_read_2 copy_to_user(buf,bufr2,tsend=%d) with endcond==1\n",tsend); 
#endif
		return -EFAULT;
	  }
  }
#ifdef DEBUG_PRINT
  printk(KERN_INFO "ETH2 schar_read_2 read: %d endcond: %d\n", count, endcond ); 
#endif
  return count;
}
...
#endif
	// schar_root_dir_2->child->de->fill_inode = &schar_fill_inode_2;
...
#elif SLC_VERSION == 7
  dev=dev_get_by_name(&init_net,"enp5s0f0");
#endif
  err=-ENODEV;
  if (dev == NULL)
   goto out_unlock;
...
#ifdef DEBUG_PRINT
  printk(KERN_INFO "ETH2 schar_write_2 count: %d\n", count);
  printk(KERN_INFO "ETH2 device name: %s hard_header_len: %d\n", dev->name, dev->hard_header_len);
  /* printk(KERN_INFO "eth_hook_2_vme BITS_PER_LONG=%d\n", BITS_PER_LONG); */
#endif
...
*/
  ...
  print_skb( "ETH2 after dev_alloc_skb", skb );
/*
...
*/
   ...
   print_skb( "ETH2 after skb_reserve", skb );
#if   SLC_VERSION == 5
   skb->nh.raw = skb->data;
#else
   skb_reset_network_header(skb);
#endif
   print_skb( "ETH2 after skb_reset_network_header", skb );
...
#endif
			print_skb( "ETH2 after skb_reset_tail_pointer", skb );
...
/* Returns -EFAULT on error */
   ...
   print_skb( "ETH2 after copy_from_user", skb );
