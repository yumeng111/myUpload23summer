...
ConfigurationEditor::ConfigurationEditor(xdaq::ApplicationStub * s) throw (xdaq::exception::Exception) :
  xdaq::Application(s)
{
  xgi::bind(this, &ConfigurationEditor::Default, "Default");
  xgi::bind(this, &ConfigurationEditor::parseConfigFromXML, "parse");
  ...
  xgi::bind(this, &ConfigurationEditor::changeSingleValue, "changeSingleValue");
  xgi::bind(this, &ConfigurationEditor::showTable, "Show");
  xgi::bind(this, &ConfigurationEditor::hideTable, "Hide");
  xgi::bind(this, &ConfigurationEditor::selectVersions, "selectVersions"); //select two versions to compare
...
void ConfigurationEditor::uploadConfFile(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  try
  {
    cgicc::Cgicc cgi(in);
...
void ConfigurationEditor::SelectLocalConfFile(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  try
  {
...
void ConfigurationEditor::outputFileSelector(xgi::Output * out) throw (xgi::exception::Exception)
{
  ...
  try
  {
    *out << "<table border=\"0\" cellpadding=\"3\"><tr><td align=\"right\">";
    *out << form().set("method", "POST").set("action", urn + "selectConfFile") << endl;
    *out << "enter full path of file on server: </td><td>"
         << input().set("type", "text").set("name", "xmlFilename").set("size", "90").set("ENCTYPE", "multipart/form-data").set("value", xmlfile_) << endl;
    *out << input().set("type", "submit").set("value", "Select") << endl;
    *out << form() << endl;

    // Upload file...
    *out << "</tr><tr><td align=\"right\">"
         << form().set("method", "POST").set("enctype", "multipart/form-data").set("action", urn + "UploadConfFile") << endl;
    *out << "or upload file:  </td><td>"
         << input().set("type", "file").set("name", "xmlFilenameUpload").set("size", "90") << endl;
    *out << input().set("type", "submit").set("value", "Upload") << endl;
    *out << form() << endl;

    *out << "</tr><tr><td align=\"right\">" << form().set("method", "POST").set("action", urn + "selectConfFile") << endl;
    ...
    *out << "or select existing file:  </td><td>"
         << cgicc::select() .set("id", "xml_file_select") .set("class","file_select") .set("name", "xmlFilename") << endl;
    ...
    *out << "</tr><tr><td align=\"right\">" << "Selected XML filename: </td><td><i>" << xmlfile_ << endl;

    *out << cgicc::br() << "</td></tr></table>";
  }
...
{
  std::string js_header = "<script language=\"javascript\" src=\"/emu/emuDCS/OnlineDB/html/pc_config_editor.js\"></script>";

  emu::utils::headerXdaq(out, this, "Configuration Editor","", js_header);
...
void ConfigurationEditor::Default(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  try
  {
...
void ConfigurationEditor::addTable(const std::string &tableName)
{
...
void ConfigurationEditor::addChildTable(const std::string &parentTableName, const std::string &childTableName)
{
...
std::string ConfigurationEditor::fullTableID(const std::string &configName, const std::string &identifier)
{
  return configName + "_" + identifier;
}


bool ConfigurationEditor::shouldDisplayConfiguration(const std::string &configName, const std::string &identifier)
{
...
void ConfigurationEditor::outputShowHideButton(
    std::ostream * out,
...
void ConfigurationEditor::displayConfiguration(
    xgi::Output * out,
    const std::string &configName,
    const std::string &identifier)
{
...
void ConfigurationEditor::sumChanges(TableChangeSummary &allChanges, TableChangeSummary &changesToThisTable)
{
...
void ConfigurationEditor::displayDiff(
    std::ostream * out,
    const std::string &configName,
    const std::string &identifier,
...
std::string ConfigurationEditor::elementNameFromTableName(const std::string &tableName)
{
...
bool ConfigurationEditor::shouldDisplayInHex(const std::string &columnName)
{
  return false; //should be overridden by subclasses
}


void ConfigurationEditor::copyTableToAttributes(
...
void ConfigurationEditor::addChildNodes(
...
void ConfigurationEditor::exportAsXML(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
...
bool ConfigurationEditor::displayCommonTableElements(
    std::ostream * out,
    const std::string &configName,
    const std::string &identifier,
...
std::string ConfigurationEditor::displayName(
    const std::string &configName,
    const std::string &identifier,
    xdata::Table &data)
{
...
void ConfigurationEditor::displayChildDiff(
    std::ostream * out,
    const std::string &configName,
    const std::string &parentIdentifier,
    ChangeSummary &changes)
...
bool ConfigurationEditor::moreThanOneChildConfigurationExists(
    const std::string &configName,
    const std::string &parentIdentifier)
{
  std::map<std::string, xdata::Table> &tables = currentDiff[configName];
...
void ConfigurationEditor::displayChildConfiguration(
    xgi::Output * out,
    const std::string &configName,
    const std::string &parentIdentifier)
{
...
void ConfigurationEditor::outputCurrentConfiguration(xgi::Output * out)
{
  //if (TStore_myEndcap_) {
...
void ConfigurationEditor::outputFooter(xgi::Output * out)
{
...
void ConfigurationEditor::outputException(xgi::Output * out, xcept::Exception &e)
{
...
void ConfigurationEditor::outputSingleValue(
    std::ostream * out,
...
std::string ConfigurationEditor::withoutVersionNumber(const std::string &columnName)
{
...
bool ConfigurationEditor::getVersionNumber(const std::string &columnName, std::string &versionNumber)
{
  size_t space = columnName.find(' ');
...
std::string ConfigurationEditor::newCell(xdata::Serializable *newValue, xdata::Serializable *oldValue)
{
...
bool ConfigurationEditor::getNextColumn(
...
void ConfigurationEditor::outputDiffRow(
    std::ostream * out,
    xdata::Table &results,
    int rowIndex,
    bool vertical,
...
void ConfigurationEditor::outputDiffSummary(std::ostream *out, TableChangeSummary &changes)
{
...
void ConfigurationEditor::outputDiffSummary(std::ostream *out, ChangeSummary &changes)
{
...
void ConfigurationEditor::outputDiff(
    std::ostream * out,
    xdata::Table &results,
    const std::string &tableName,
    TableChangeSummary &changes)
...
void ConfigurationEditor::outputTable(
    std::ostream * out,
    xdata::Table &results,
    const std::string &tableName,
    const std::string &identifier)
...
void ConfigurationEditor::getTableDefinitionsIfNecessary() throw ()
{
  try
  {
...
void ConfigurationEditor::showHideTable(xgi::Input * in, xgi::Output * out, bool show) throw (xgi::exception::Exception)
{
  cgicc::Cgicc cgi(in);
...
void ConfigurationEditor::showTable(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  showHideTable(in, out, true);
}


void ConfigurationEditor::hideTable(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  showHideTable(in, out, false);
}


void ConfigurationEditor::compareVersions(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  clearCachedDiff();
  try
  {
...
std::string ConfigurationEditor::configIDOptions(std::vector<std::string> &configIDs)
{
...
std::string ConfigurationEditor::configIDOptionsWithDescription(std::vector<ConfigIDInfo> &configIDs)
{
  // sort in reverse order, so the newest are on the top
  //std::sort(configIDs.begin(), configIDs.end());
  //std::reverse(configIDs.begin(), configIDs.end());
...
void ConfigurationEditor::outputCompareVersionsForm(xgi::Output * out, const std::string &endcap_side)
{
  try
  {
...
void ConfigurationEditor::selectVersions(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  cgicc::Cgicc cgi(in);
...
void ConfigurationEditor::selectVersion(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  try
  {
    cgicc::Cgicc cgi(in);
    std::string endcap_side = **cgi["side"];
    //std::vector<std::string> configIDs;
    std::vector<ConfigIDInfo> configIDs;
    getConfigIdInfos(configIDs, endcap_side);
    outputHeader(out);
    outputStandardInterface(out);
    ...
    *out << "read version " << cgicc::select().set("name", "configID")
        << configIDOptionsWithDescription(configIDs) << cgicc::select();
...
void ConfigurationEditor::setValueFromString(xdata::Serializable *value, const std::string &newValue)
...
std::string ConfigurationEditor::copyAttributesToTable(
    xdata::Table &table,
    const std::string &tableName,
    xercesc::DOMElement *node,
    int rowIndex) throw (xcept::Exception)
...
std::string ConfigurationEditor::uniqueIdentifierForRow(
    xdata::Table &table,
    const std::string &tableName,
...
void ConfigurationEditor::changeSingleValue(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  cgicc::Cgicc cgi(in);
  std::string tableName = **cgi["table"];
...
void ConfigurationEditor::getRangeOfTables(
...
bool ConfigurationEditor::tableHasColumn(xdata::Table &table, const std::string &column)
{
  std::vector<std::string> columns = table.getColumns();
...
void ConfigurationEditor::parseConfigFromXML(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  //actually, I should write a generic version of this
  //indeed! In progress. Stand by.
  //ConfigurationEditor::getTableDefinitionsIfNecessary();
...
void ConfigurationEditor::uploadConfigToDB(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  try
  {
    outputHeader(out);
...
//to be implemented by subclasses
void ConfigurationEditor::readConfigFromDB(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  try
  {
    cgicc::Cgicc cgi(in);
...
void ConfigurationEditor::synchronizeToFromDB(xgi::Input * in, xgi::Output * out) throw (xgi::exception::Exception)
{
  try
  {
    outputHeader(out);
...
void ConfigurationEditor::getConfigIds(std::vector<std::string> &configIDs, const std::string endcap_side)
...
//gets all ConfigIDInfos for the given endcap side
void ConfigurationEditor::getConfigIdInfos(std::vector<ConfigIDInfo> &configIDs, const std::string endcap_side)
    throw (xcept::Exception)
{
  // check that configuration hierarchy was initialized:
  if (hierarchy_ == 0)
  {
    XCEPT_RAISE (xcept::Exception, "ConfigurationEditor::getConfigIdInfos: hierarchy_ vas not initialized!");
  }
  ...
  for (unsigned long rowIndex = 0; rowIndex < results.getRowCount(); rowIndex++)
  {
    configIDs.push_back( ConfigIDInfo(results, rowIndex, hierarchy_) );
  }
}
...
void ConfigurationEditor::setPassword(const std::string &password)
{
  password_ = password;
...
void ConfigurationEditor::setUsername(const std::string &username)
{
  username_ = username;
  createCredentialString();
}
...
void ConfigurationEditor::createCredentialString()
{
...
/*
void ConfigurationEditor::query(const std::string &connectionID, const std::string &queryViewName,
...
void ConfigurationEditor::query(
...
void ConfigurationEditor::getDefinition(
    const std::string &connectionID,
...
void ConfigurationEditor::insert(
    const std::string &connectionID,
...
void ConfigurationEditor::synchronize(
    const std::string &connectionID,
...
void ConfigurationEditor::getTableDefinitions(const std::string &connectionID)
{
...
void ConfigurationEditor::setCachedTable(
    const std::string &insertViewName,
    const std::string &identifier,
    xdata::Table &table) throw (xcept::Exception)
{
  std::ostringstream logMessage;
  logMessage << "setting cached " + insertViewName + " table " + identifier + " with " << table.getRowCount()
      << " rows";
...
void ConfigurationEditor::setCachedDiff(
    const std::string &insertViewName,
    const std::string &identifier,
    xdata::Table &table) throw (xcept::Exception)
{
...
xdata::Table &ConfigurationEditor::getCachedTableFrom(
...
xdata::Table &ConfigurationEditor::getCachedTable(
    const std::string &insertViewName,
    const std::string &identifier
    /*,xdata::UnsignedInteger64 &_vcc_config_id*//*,Crate *thisCrate*/) throw (xcept::Exception)
{
...
xdata::Table &ConfigurationEditor::getCachedDiff(
    const std::string &insertViewName,
    const std::string &identifier
    /*,xdata::UnsignedInteger64 &_vcc_config_id*//*,Crate *thisCrate*/) throw (xcept::Exception)
{
...
void ConfigurationEditor::clearCachedTables()
{
  currentTables.clear();
}


void ConfigurationEditor::clearCachedDiff()
{
  currentDiff.clear();
}
