...
//
template <size_t XS_per_CFEB> int ChamberUtilities::GetInputXStrip(int output_xstrip) {
  ...
  return input_xs;
    ...
    //
template <size_t XS_per_CFEB>  int ChamberUtilities::GetInputCFEBByX(int output_xstrip) {
  const int Min_CFEB_in_non_me11 = 0;
  const int Max_CFEB_in_non_me11 = 4;
  const int Min_CFEB_in_region_a = 0;
  const int Max_CFEB_in_region_a = 3;
...
//
template <size_t XS_per_CFEB> int ChamberUtilities::GetOutputXStrip(int cfeb, int input_xstrip) {
  const int Bad_HS = -1;
  const int Min_CFEB_in_non_me11 = 0;
  const int Max_CFEB_in_non_me11 = 4;
...
//    thisCCB_->setCCBMode(CCB::DLOG);
  ...
  for(int cfeb_phase = (is_cfeb_clock_phase_scan)?(0):(cfeb_clock_phase); 
	...
	SetCfebRxPosNeg(posneg);
      //
      for (int TimeDelay = (is_timing_scan)?(0):(time_delay); 
	   ((is_timing_scan)?(TimeDelay<MaxTimeDelay):(TimeDelay<=time_delay)) && (!done);
	   ++TimeDelay) { 
...
//	      thisCCB_->setCCBMode(CCB::DLOG);
  ...
  if(is_cfeb_clock_phase_scan) {
	...
	web_backup << std::setw(5) << TimeDelay;
      ...
      for (int cfeb_phase = (is_cfeb_clock_phase_scan)?(0):(cfeb_clock_phase); 
	   ((is_cfeb_clock_phase_scan)?(cfeb_phase<MaxCFEBClockPhase):(cfeb_phase==cfeb_clock_phase));
	   ++cfeb_phase) {
  ...
  if(is_me11_) {
	...
	if(good_cfebs[side] == ((side==0)?4:3)) {
	  ...
	  if(side==0){
	    for(int rx=0; rx<25; ++rx){
	      for(int cfeb=0; cfeb<4; ++cfeb) total_error_count_rx[side][posneg][rx] += pulse_count_cfeb_rx[cfeb][posneg][rx]; // for me1/1b
	    }
	  }
	  else {
	    for(int rx=0; rx<25; ++rx){
	      for(int cfeb=4; cfeb<7; ++cfeb) total_error_count_rx[side][posneg][rx] += pulse_count_cfeb_rx[cfeb][posneg][rx]; // for me1/1a
	    }
	  }
	}
	// Okay Case
	  ...
	  if(side==0){
	    for(int rx=0; rx<25; ++rx){
	      for(int cfeb=0; cfeb<4; ++cfeb) total_error_count_rx[side][posneg][rx] += (cfeb==bad_cfeb_b)?0:pulse_count_cfeb_rx[cfeb][posneg][rx]; // for me1/1b
	    }
	  }
	  else {
	    for(int rx=0; rx<25; ++rx){
	      for(int cfeb=4; cfeb<7; ++cfeb) total_error_count_rx[side][posneg][rx] += (cfeb==bad_cfeb_a)?0:pulse_count_cfeb_rx[cfeb][posneg][rx]; // for me1/1a
	    }
	  }
	}
	// Fail Case
