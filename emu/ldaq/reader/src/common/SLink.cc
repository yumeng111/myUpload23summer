#include "emu/ldaq/reader/SLink.h"
...
emu::ldaq::reader::SLink::SLink( int boardIndex,  int format, bool debug )
  : emu::ldaq::reader::Base( format, debug ),
...
emu::ldaq::reader::SLink::~SLink(){
  close();
}

void emu::ldaq::reader::SLink::open() {
...
void emu::ldaq::reader::SLink::close() {
  fedkit_close (receiver);
}


int emu::ldaq::reader::SLink::reset(void){
  fedkit_close(receiver);
  sleep(1);
  receiver = fedkit_open(0, NULL);
  if (receiver == NULL) {
    std::cout << "emu::ldaq::reader::SLink::reset: FAILED TO INITIALIZE. Aborting ..." << std::endl;
    abort();
  }
  fedkit_start (receiver);
  return 0;
...
int emu::ldaq::reader::SLink::chunkSize() {
  //int cnt[2];
...
int emu::ldaq::reader::SLink::readDDU(unsigned short **buf) {
  // TODO
  return -1;
}

int emu::ldaq::reader::SLink::readDMB(unsigned short **buf) {
  // DMB has no SLink.
  return -1;
}

int emu::ldaq::reader::SLink::readDCC(unsigned short **buf) {
  char b[50000];
  char *c,*d;
