...
// DEBUG START
  ...
  ec1 = new emu::ldaq::reader::Clock(100);
  ec2 = new emu::ldaq::reader::Clock(100);
  ec3 = new emu::ldaq::reader::Clock(100);
  ec4 = new emu::ldaq::reader::Clock(100);
// DEBUG END
  open( filename );
// DEBUG START
  visitCount =  0;
...
void emu::ldaq::reader::Spy::open(std::string filename) {
  theLogMessage = "";
  theFileDescriptor = ::open(filename.c_str(), O_RDONLY);
  if ( theDebugMode ){
    if (filename.find("/dev/schar")==0)
      std::cout << "emu::ldaq::reader::Spy::open: we have got a life one here, Jimmy: " << filename << std::endl;
  ...
  if ( theDebugMode ){
    if (buf_start==MAP_FAILED) {
      std::cerr << "emu::ldaq::reader::Spy::open: FATAL in memorymap - " << std::strerror(errno) << std::endl;
      ...
      std::cerr << "emu::ldaq::reader::Spy::open will abort!!!" << std::endl;
      abort();
    }
    std::cout << "emu::ldaq::reader::Spy::open: Memory map succeeded " << std::endl;
  }
  if (buf_start==MAP_FAILED){
    std::stringstream ss;
...
void emu::ldaq::reader::Spy::close() {
#ifdef USE_DDU2004
...
int emu::ldaq::reader::Spy::reset(void){
  int status;
...
int emu::ldaq::reader::Spy::enableBlock(void){
  int status;
...
int emu::ldaq::reader::Spy::disableBlock(void){
  int status;
...
int emu::ldaq::reader::Spy::endBlockRead(){
  int status;
...
int emu::ldaq::reader::Spy::chunkSize() {
  //int cnt[2];
...
void emu::ldaq::reader::Spy::resetAndEnable(){
  theLogMessage = "";
  reset();
  // enableBlock();
...
int emu::ldaq::reader::Spy::readDCC(uint16_t*& buf) {
  return readDDU( buf );
}

int emu::ldaq::reader::Spy::readDMB(uint16_t*& buf) {
  return readDDU( buf );
}

int emu::ldaq::reader::Spy::dataLengthWithoutPadding( const uint16_t* data, const int dataLength ){
