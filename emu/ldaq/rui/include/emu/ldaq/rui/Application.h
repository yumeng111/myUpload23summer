...
#include "emu/ldaq/rui/exception/Exception.h"
...
#include "emu/ldaq/server/i2oMsg.h"
#include "emu/ldaq/writer/RawDataFile.h"
#include "emu/ldaq/writer/RateLimiter.h"
#include "emu/ldaq/rui/i2oFirstEventNumberMsg.h"
...
private:
  ...
  FEROLDataStream *FEROLDataStream_;
  ...
  uint32_t                            inputDataFormatInt_;   // emu::ldaq::reader::Base::DDU or emu::ldaq::reader::Base::DCC
  ...
  emu::ldaq::writer::RateLimiter       *rateLimiter_;
  ...
  vector< const xdaq::ApplicationDescriptor* > getAppDescriptors(const xdaq::Zone *zone,
							         const string      appClass)
    throw (emu::ldaq::rui::exception::Exception);

  time_t toUnixTime( const std::string YYMMDD_hhmmss_UTC );

  // void getRunInfo() throw (emu::ldaq::rui::exception::Exception);
  ...
  emu::base::FactCollection findFacts();
     ...
     */
    const xdaq::ApplicationDescriptor *rubuilderTesterDescriptor_;
...
/*     vector<toolbox::mem::Reference*> superFragBlocks_; */
     ...
     */
    const xdaq::ApplicationDescriptor *getRUBuilderTester
    (
  ...
  ///
    ...
    // throw (emu::ldaq::rui::exception::Exception);
    int32_t continueConstructionOfFragment()
    throw (emu::ldaq::rui::exception::Exception);
...
//     void setNbBlocksInSuperFragment(const unsigned int nbBlocks);
     ...
     */
    string getHref(const xdaq::ApplicationDescriptor *appDescriptor);
