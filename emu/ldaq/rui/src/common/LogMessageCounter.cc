...
emu::ldaq::rui::LogMessageCounter::~LogMessageCounter(){
  reset();
}

void emu::ldaq::rui::LogMessageCounter::reset(){
  for ( map<string, uint64_t*>::iterator i = counts_.begin(); i != counts_.end(); ++i ) delete i->second;
  counts_.clear();
}

uint64_t emu::ldaq::rui::LogMessageCounter::increment( const string& message ){
...
uint64_t emu::ldaq::rui::LogMessageCounter::getCount( const string& message ) const {
   map<string, uint64_t*>::const_iterator i = counts_.find( message );
   if ( i !=  counts_.end() ) return *i->second;
   return 0;
}

bool emu::ldaq::rui::LogMessageCounter::isPassingProgressivePrescaling( uint64_t count ) const {
...
bool emu::ldaq::rui::LogMessageCounter::isToBeLogged( const string& message ){
