...
emu::ldaq::server::Base::Base( xdaq::Application                    *parentApp,
...
// emu::ldaq::server::Base::~Base(){} this must go to the .h file, otherwise the executive won't load this lib module

string emu::ldaq::server::Base::createName(){
  ostringstream oss;
...
void emu::ldaq::server::Base::findClientDescriptor()
{
  try
    {
      clientDescriptor_ =
...
void emu::ldaq::server::Base::addCredits( const int nCredits, const int prescaling ){
