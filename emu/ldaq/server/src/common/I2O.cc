...
emu::ldaq::server::I2O::I2O( xdaq::Application                    *parentApp,
  ...
  : emu::ldaq::server::Base ( parentApp,
			     clientName,
			     clientInstance,
			     prescaling,
			     onRequest,
...
void emu::ldaq::server::I2O::findClientTid(){
...
void emu::ldaq::server::I2O::createCommittedHeapAllocatorMemoryPool()
  throw ( xcept::Exception )
{
...
void emu::ldaq::server::I2O::addData( const int               runNumber, 
...
//     LOG4CPLUS_DEBUG(logger_, "emu::ldaq::server::I2O::addData: " << messageQueue_.size() << " data blocks pending.");

}


void emu::ldaq::server::I2O::createMessage( toolbox::mem::Reference *bufRef,
					   const int               runNumber, 
					   const int               runStartUTC,
					   const PositionInEvent_t position, 
					   const uint16_t          errorFlag, 
  ...
  throw ( xcept::Exception )
{
    ...
    messageQueue_.push_back( pair< toolbox::mem::Reference*, emu::ldaq::server::PositionInEvent_t >( bufRef, position ) );

    if ( position & emu::ldaq::server::endsEvent ) ++nEventsInQueue_;
    
}

void emu::ldaq::server::I2O::fillBlock( toolbox::mem::Reference *bufRef,
				       const int               runNumber, 
				       const int               runStartUTC,
				       const PositionInEvent_t position, 
				       const uint16_t          errorFlag, 
...
void emu::ldaq::server::I2O::insertNumberOfCreditsHeld( toolbox::mem::Reference *bufRef, PositionInEvent_t position ){
  ...
  if ( position & emu::ldaq::server::endsEvent )
...
void emu::ldaq::server::I2O::sendData()
  throw ( xcept::Exception )
{
  while ( isEventEndInQueue() ){
    if ( ( messageQueue_.front().second & emu::ldaq::server::startsEvent ) && 
	 ( messageQueue_.front().second & emu::ldaq::server::endsEvent   )    ){
...
void emu::ldaq::server::I2O::makeLastBlockEndEvent(){
  ...
  deque<pair<toolbox::mem::Reference*,emu::ldaq::server::PositionInEvent_t> >::reverse_iterator lastBlock = messageQueue_.rbegin();
  lastBlock->second = (emu::ldaq::server::PositionInEvent_t)( lastBlock->second | emu::ldaq::server::endsEvent );
  ++nEventsInQueue_;
}

bool emu::ldaq::server::I2O::isEventEndInQueue(){
  // Returns TRUE if any data blocks with an "endsEvent" mark are still in the queue.
  for ( deque<pair<toolbox::mem::Reference*,emu::ldaq::server::PositionInEvent_t> >::iterator msg = messageQueue_.begin(); msg != messageQueue_.end(); ++msg ){
    if ( msg->second & emu::ldaq::server::endsEvent ) return true;
  }
  return false;
}

void emu::ldaq::server::I2O::sendNextMessageInQueue(){
...
void emu::ldaq::server::I2O::sendMergedEventMessage(){
  ...
  deque<pair<toolbox::mem::Reference*,emu::ldaq::server::PositionInEvent_t> >::iterator messagePastEventEnd = messageQueue_.begin();
  ...
  for ( deque<pair<toolbox::mem::Reference*,emu::ldaq::server::PositionInEvent_t> >::iterator msg = messageQueue_.begin(); msg != messageQueue_.end(); ++msg ){
    ...
    // Combine from emu::ldaq::reader the error bits that are not specific to the start or end of event:
    combinedErrorFlag |= emuMsgFrame->errorFlag & ( emu::ldaq::reader::Spy::Timeout         |
						    emu::ldaq::reader::Spy::PacketsMissing  |
						    emu::ldaq::reader::Spy::LoopOverwrite   |
						    emu::ldaq::reader::Spy::BufferOverwrite |
						    emu::ldaq::reader::Spy::Oversized         );
    ...
    if ( msg == messageQueue_.begin() ){
      ...
      combinedErrorFlag |= emuMsgFrame->errorFlag & emu::ldaq::reader::Spy::HeaderMissing;
    }
    if ( msg->second & emu::ldaq::server::endsEvent ){
      // Marked as the block ending the event. Did the reader find the end of event? Does it have the DDU trailer?
      combinedErrorFlag |= emuMsgFrame->errorFlag & ( emu::ldaq::reader::Spy::EndOfEventMissing | 
						      emu::ldaq::reader::Spy::TrailerMissing      );
  ...
  for ( deque<pair<toolbox::mem::Reference*,emu::ldaq::server::PositionInEvent_t> >::iterator msg = messageQueue_.begin(); msg != messagePastEventEnd; ++msg ){
...
string emu::ldaq::server::I2O::errorFlagToString( U16 errorFlag ){
  stringstream ss;
