...
emu::ldaq::writer::RateLimiter::RateLimiter( const uint64_t limitInHz, const uint64_t sampleSize ) :
  ...
  eventCount_( 0 )
{
  ...
  endOfVetoPeriod_.tv_sec  = time_t     (0); 
  endOfVetoPeriod_.tv_usec = suseconds_t(0);
}

emu::ldaq::writer::RateLimiter::~RateLimiter(){
  delete [] timeStamps_;
}

bool emu::ldaq::writer::RateLimiter::acceptEvent(){
  ...
  struct timeval earliestEndOfSample = {
    timeStamps_[firstTimeIndex_].tv_sec  + (time_t     )(minTimeSpanInMicrosec_ / 1000000),
    timeStamps_[firstTimeIndex_].tv_usec + (suseconds_t)(minTimeSpanInMicrosec_ % 1000000)
  };
