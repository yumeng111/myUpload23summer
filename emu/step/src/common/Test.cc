...
void ( emu::step::Test::* emu::step::Test::getProcedure( const string& testId, State_t state ) )(){
  ...
  if ( testId == "40a" ) return ( state == forConfigure_ ? &emu::step::Test::configure_40   : &emu::step::Test::enable_40   );
  if ( testId == "40p" ) return ( state == forConfigure_ ? &emu::step::Test::configure_40   : &emu::step::Test::enable_40   );
  if ( testId == "40s" ) return ( state == forConfigure_ ? &emu::step::Test::configure_40   : &emu::step::Test::enable_40   );
  return NULL;
}
...
void emu::step::Test::createEndcap( const string& generalSettingsXML,
  ...
  // Save XML in file:
  string fileName( "/tmp/VME_" + withoutChars( " \t:;<>'\",?/~`!@#$%^&*()=[]|\\", group_ ) + "_Test" + id_ + ".xml" );
...
void emu::step::Test::enable_40(){
  // It would be the same as enable_27, so use that one instead to avoid code duplication.
  enable_27();
}
