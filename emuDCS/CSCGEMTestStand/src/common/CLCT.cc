...
bool compareCLCT(CLCT c1, CLCT c2)
	...
	int  GetCFEB(int hs, int COMPILE_TYPE = 0xc)
			{
				return GetInputCFEBByX<CSCConstants::NUM_HSPERDCFEB>(hs, COMPILE_TYPE);
			}

	int  GetLocal(int hs, int COMPILE_TYPE = 0xc)
	  	{
				return GetInputXStrip<CSCConstants::NUM_HSPERDCFEB>(hs, COMPILE_TYPE);
			}
...
void ExtractHits(std::vector<CLCT>& clcts, std::vector<Hit>& hits, int feb)
	{
		...
		else if(feb >= 0 && feb < CSCConstants::NUM_DCFEBS_ME11)
		{
			for(int i=0; i < clcts.size(); i++)
			{
...
bool WritePat(std::string & prefix, std::vector<CLCT>& clcts)
	{
		std::cout <<"using WritePat with CLCT version, clcts size " << clcts.size() << std::endl;
...
bool WritePat(std::string& prefix, std::vector<Cluster>& in_pads)
	{
		...
		std::vector<std::fstream*> oss;
		for(int gem_fiber = 0; gem_fiber < CSCConstants::NUM_FIBERS_GE11; gem_fiber++){
			std::stringstream ss;
...
bool WritePat(std::string& prefix, std::vector<Cluster>& in_pads)
	{
		// Open (.pat) files
		std::cout <<"using WritePat with GEM Pattern version,in_pads size " << in_pads.size() << std::endl;
		//char tmbtype = COMPILE_TYPE - 0xa + 'a';
		std::vector<std::fstream*> oss;
		/*
...
bool WritePat(std::string & prefix, std::vector<CCLUT>& ccluts)
	{
		// Prepare output file streams
		std::cout <<"using WritePat with CCLUT version, ccluts size " << ccluts.size() << std::endl;
		std::vector<std::fstream*> oss;
		char tmbtype;
