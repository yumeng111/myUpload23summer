...
namespace emu {  namespace pc {
...

  {
    ...
    */
    ...
    sprintf(SimulatedFileName, "/home/cscdev/yumeng/injectCSCPatterns/data/ComparatorDigi_CMSSW1401_CLCT_ME11_1000.txt");
...
/*
    ...
    *out << "bx keystrip pattern nhits\t{ (bx,hs,layer) }" << endl;
    for(int i=0; i < patternSet.CSCCLCTs.size(); i++){
      *out << patternSet.CSCCLCTs[i] << endl;
    }
    ...
    *out << "bx\troll\tpad\tsize\tlayer\trealVfatID" << endl;
    for(unsigned int i=0; i < patternSet.GEMClusters.size(); i++){
      ...
      *out << patternSet.GEMClusters[i] <<" \t "<< std::bitset<14>(clusterbits)<< endl;
    }
...
void CSCGEMTestApplication::RunParamScan(xgi::Input * in, xgi::Output * out)
{
  ...
  do{
    ...
    for(int i=0; i < CLCT_ps_vec.size(); i++){
      patternSet.CSCCLCTs.push_back(CLCT_ps_vec[i]);
    }
...
{
  cgicc::Cgicc cgi(in);
  // newfile.open(/home/cscdev/yumeng/injectCSCPatterns/data/ComparatorDigi_CMSSW1401_CLCT_ME11_000.txt)
...
void CSCGEMTestApplication::AutomaticCheckingFile(xgi::Input * in, xgi::Output * out ){
  ...
  do {
    ...
    if (word == initWord){
      ...
      while(word != "Run"  && word != "CSC" /*&& word != "CSCChamber"*/ && SimulatedFile >> word){
        if (word == "Comparatordigi") { // This is supposed to check for the BX,layer,halfstrip when it comes after "Comparatordigi"
          ...
          std::cout << "BX = " << bx_num << "; Layer = " << layer_num << "; halfstrip = " << halfstrip_num << std::endl;
          if (bx_num >= clctFifoTbins - 2) continue; //ignore the comparator digis coming too late
          cw::Hit tmp_hit;
          tmp_hit.bx = bx_num;
          tmp_hit.lay = layer_num;
          tmp_hit.hs = halfstrip_num;
    ...
    for (int i = 0; i < NInjections; ++i) {
      ...
      patternSet.SetOTMBCompileType(GetOTMBType[0]);
      cw::CCLUT tmp_cclut;
      tmp_cclut.SetOTMBCompileType(0xa+GetOTMBType[0]-'a');
      tmp_cclut.hits = hits;
      patternSet.LUT.push_back(tmp_cclut);
