...
void emu::farmer::Application::generateConfigFiles()
  ...
  for ( size_t iSide=0; iSide<sizeof(sides)/sizeof(char*); ++iSide ){
    //
    // DUCK files for DQM
    //
    ...
    for ( size_t iBuild=0; iBuild<sizeof(build)/sizeof(char*); ++iBuild ){
      for ( size_t iWrite=0; iWrite<sizeof(write)/sizeof(char*); ++iWrite ){
...
emu::farmer::Application::recreateMapping( std::vector<cgicc::FormEntry>& fev )
  ...
  try{
    ...
    for ( map<string,string>::size_type iComp = 0; iComp < aliases.size(); ++iComp ){
      stringstream comp;
      comp << "c" << iComp;
	...
	for ( map<string,string>::size_type iRUI = 0; iRUI < instances.size(); ++iRUI ){
	  stringstream rui;
